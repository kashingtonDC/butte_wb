    ...
    if polarea < 500**2:
        ...
        try:
            ...
            for y in years[:]:
                ...
                petmask = np.isin(nlcd_im, [11, 82], invert = False).reshape(nlcd_im.shape).astype(int)
                aetmask = np.isin(nlcd_im, [11, 82], invert = True).reshape(nlcd_im.shape).astype(int)
...
def main():
    ...
    kc = pd.read_csv('../data/fmp_kc_faunt.csv')
    gdf = gp.read_file('../shape/study_area/c2vsim_subregion5.shp')
    data = rs.load_data()
